...
| 3.1 | Purpose (unified LLM API) | ✅ | All seven providers shipped: `faux`, `anthropic`, `openai_chat`, `openai_responses`, `openai_gateway`, `google_gemini`, `google_vertex`. Registry dispatch working in print + rpc + proxy modes |
...
| 6 | TUI library | ❌ removed in #38 | `src/tui/` complete; raw-terminal binding via `src/coding/terminal.zig` wired through `src/coding/modes/interactive.zig`. Removed together with interactive mode in PR #38 (tool-management consolidation, −6 189 lines). The web UI (§7) is the remaining interactive surface. |
...
| G.5 | Logging & tracing | ✅ | `src/ai/log.zig` — 5 levels, atomic threshold. **v1.13.0** adds `initWithFile(io, level, path)` so logs route to a file instead of stderr; new `--log-file PATH` CLI flag + `FRANKY_LOG_FILE` env var; proxy mode auto-diverts to `$FRANKY_HOME/logs/franky-<unix_ms>.log` whenever level > `warn` and no explicit file is set, with a stderr banner so users know where to tail. Solves the UI-vs-stderr conflict where `--mode proxy --log-level trace` would garble the web UI output on the same TTY (interactive-mode auto-divert removed in #38) |
...
| L | TUI rendering | ❌ removed in #38 | Differential renderer + raw-terminal binding + interactive dispatch all lived in v1.x; removed with interactive mode in PR #38. |
...
`SANDBOX.md` (zerobox primary, Docker, devcontainer, Lima,
