...
rpc/proxy use the stateless bash factory and don't
...
`applyPermissionsSettingsOverlay`. Wired in all three modes;
...
recipes per v1 §5.10 and `SANDBOX.md`); no in-process firewall.
...
6. **Sub-agent `final_text_max_bytes` + transcript-on-disk firewall.**
   ...
   details lookup-on-demand). +1 test (`successResult emits transcript_path
...
7. **Bash spill path tied to session dir.** ✅ shipped v1.27.2
   ...
   - **interactive mode (removed in #38):** Interactive mode was
     removed in PR #38 (tool-management consolidation). The bash
     spill path now applies only to print/rpc/proxy, all of which
     persist sessions on disk.
...
5. **Typed `error_ev` recovery taxonomy.** Promote
   ...
   Tier-2 also covers the proxy + rpc plumbing of
...
v1.17.0 entry in the v1 spec's "what shipped" log
...
- Move this §5 entry from this file into `v1.md`
  ...
  shipped at v1.17.0.
- ~~CHANGELOG entry.~~ (CHANGELOG.md removed; per-version history lives in the v1/v2 spec row tables.)
...
- **§4.5 + §4.10 + §4.12 — settings-layer overlay** → shipped
  ...
  print (per-call `limit` arg still wins). New
  ...
  helpers in `print.zig`. Wired in all three modes.
...
- **v1.20.0 — proxy.zig diet (Option C, partial)**. From
  ...
  unification (interactive mode removed in #38), endpoint dispatcher,
...
instance of the running binary, and exits the current process. Two triggers
(interactive-mode `/restart` slash command removed in #38):

- **`POST /restart` HTTP route** (proxy mode) — sets `restart_requested`
  flag; the main loop polls the flag between turns, then calls
  `restart.spawnAndExit(io)`. Closes the listen socket before spawning so the
...
- **`finish_task` with `restart: true`** (model-driven) — the guardrail sets
  ...
  picked up by the proxy mode loop.
...
### 6.14 Persistent memory tools (v3.2)

Shipped. Agent-driven persistent memory via two tools backed by a
vendored SQLite store (`agent_memory` v0.0.2, SQLite 3.53.4). The
memory subsystem spans PRs #32, #34, #35, #36, #37, #40, #44.

- **`memory_search`** — `{query, limit?}` searches persistent memory
  across previous sessions; ranked by relevance.
- **`memory_save`** — `{content, type, priority?, scene_name?}` saves
  a durable fact/decision/preference/instruction. `type` is one of
  `persona | episodic | instruction`; only self-contained, durable,
  user/AI-centric entries belong here.

CLI flags (`src/coding/config/cli.zig`):

- `--memory` / `--no-memory` — enable/disable the memory tools and the
  `## Memory Tools` system-prompt section (default on; settings.json
  `tools.memory.enabled` overlays).
- `--memory-nudge` / `--no-memory-nudge` — when on, the
  `finish_task` guardrail injects a synthetic nudge prompting the
  model to `memory_save` anything worth remembering before the session
  ends (default on; no-op when memory is disabled).

Wiring: `src/coding/memory.zig` owns `MemoryState` (the
`agent_memory.SqliteStore` + context). `finalizeToolSet`
(`src/coding/config.zig`) registers `memory_search` + `memory_save`
when `memory_state != null` — this is the unified tool path shared by
**all three modes** (print, proxy, rpc), so the tools are available
everywhere after the #38 tool-management consolidation. PR #40
specifically wired them into proxy and rpc (previously print-only).

Context injection: `MemoryState.buildContextBlock(query)` runs at
session start and injects a `## Memory` section into the system prompt
with relevant recalled memories. L0 capture + the nudge guardrail run
at turn/finish boundaries.

SQLite vendoring: the `agent_memory` dependency bundles a vendored
SQLite amalgamation so franky cross-compiles without a system
`libsqlite3` (PR #35; bumped to 3.53.4 in #44).

### 6.15 Sub-agent `context_file` parameter (#39)

Shipped. The `subagent` tool gained a `context_file` parameter for
passing large payloads (e.g. a diff > ~10 KB) to a sub-agent by path
instead of inlining them into `prompt` (the parent's JSON string
emission is unreliable for long payloads).

- Schema: `context_file: ?[]const u8` on `ParsedArgs`
  (`src/coding/tools/subagent.zig`).
- The file is loaded server-side and appended to `prompt` before the
  sub-agent is spawned. Max 256 KB (`context_file_max_bytes`).
- Path-safety: when `SubagentCtx.workspace_root` is set, `context_file`
  paths are canonicalized against the workspace root with the same
  path-safety as the `read` tool; when null, paths are read directly.

### 6.16 Stream-channel dedicated-thread fix (#43)

Shipped. Correctness fix for a bounded-channel single-thread deadlock
in the agent turn loop (`src/agent/loop.zig`).

Previously `registry.stream(...)` ran synchronously on the same thread
as the drain loop — the provider pushed every SSE event into the
bounded stream channel (16 384 slots) *before* the drain started. A
large response (observed ~5.6 MB / ~28K–93K events) filled the ring
and `Channel.push` blocked forever on `not_full.wait`. Cancel could
not break it (`push` is uncancelable; the HTTP driver's cancel check
only runs between events), so interrupts silently no-op'd and the
whole agent wedged until killed.

Fix: the provider's `stream` now runs on a **dedicated thread**
(`streamWorkerMain`); `runTurn` drains the channel concurrently on the
caller thread. The ring never fills regardless of response size. The
stream thread is joined on every exit path (normal completion, error,
cancel). A post-stream peak-usage diagnostic (`stream_channel_usage` /
`stream_channel_high_watermark`) records the high-water mark.

Design notes: `docs/limits/stream-channel-capacity.md`.

### 6.17 Architecture refactor — agent→coding cycle break (#41)

Shipped. Broke the agent→coding dependency cycle and fixed leaking
abstractions. The `agent` layer no longer imports `coding`; tool
management was consolidated into `src/coding/config.zig::finalizeToolSet`
(shared by all three modes), eliminating per-mode tool-registration
duplication. Pure structural refactor — no behavioral change.
