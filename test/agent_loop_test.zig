...
// ─── v3.2 capture_turn hook tests ──────────────────────────────────

const CaptureCounter = struct {
    call_count: u32 = 0,
    total_messages: usize = 0,

    fn hook(userdata: ?*anyopaque, _: *loop.Transcript, new_messages: []const at.AgentMessage) void {
        const self: *CaptureCounter = @ptrCast(@alignCast(userdata.?));
        self.call_count += 1;
        self.total_messages += new_messages.len;
    }
};

test "v3.2 capture_turn: fires after each turn with new messages" {
    var threaded = franky.test_helpers.threadedIo();
    defer threaded.deinit();
    const io = threaded.io();
    const gpa = franky.global_allocator.gpa;
    ...
    loop.agentLoop(gpa, io, &transcript, .{
        ...
        .registry = &reg,
        .cancel = &cancel,
        .capture_turn = CaptureCounter.hook,
        .capture_turn_userdata = @ptrCast(&counter),
    }, &ch);

    while (ch.next(io)) |ev| ev.deinit(gpa);

    // The faux provider emits one assistant message; capture_turn fires
    // once with that single message.
    try testing.expectEqual(@as(u32, 1), counter.call_count);
    try testing.expectEqual(@as(usize, 1), counter.total_messages);
    try testing.expectEqual(@as(usize, 1), transcript.messages.items.len);
}

test "v3.2 capture_turn: null hook is a no-op (no crash)" {
    var threaded = franky.test_helpers.threadedIo();
    defer threaded.deinit();
    const io = threaded.io();
    const gpa = franky.global_allocator.gpa;
    ...
    // No capture_turn set — should run without crashing.
    loop.agentLoop(gpa, io, &transcript, .{
        .model = .{ .id = "faux-1", .provider = "faux", .api = "faux" },
        .tools = &.{},
        .registry = &reg,
