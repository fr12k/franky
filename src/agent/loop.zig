...
/// v3.2 — L0 capture hook. Called after each turn completes, with the
/// slice of messages that were appended to the transcript during that
/// turn (the "delta"). Implementations persist them to L0 (raw
/// conversation storage). Best-effort: the loop ignores errors from this
