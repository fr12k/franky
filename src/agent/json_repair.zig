...
//!
//! Some LLMs emit concatenated JSON objects as tool-call arguments
//! (e.g. `{"path":"a"}{"path":"b"}{"path":"c"}`). This module repairs
//! that into a single JSON object so the agent loop can parse it.
//!
//! Previously this lived in `coding/tools/common.zig`, which forced
//! `agent/loop.zig` to import a coding-layer module — a cycle
//! (`agent → coding → agent`). The repair logic is an agent-runtime
//! concern (normalizing model output before parsing), not a tool
//! concern, so it belongs in the agent layer. `coding/tools/common.zig`
//! re-exports it for backward compatibility with the many tools that
//! call `common.repairConcatJson`.
