...
// VENDORED COPY of std/crypto/tls/Client.zig from Zig
// 0.17.0-dev.1622+2b242157b with a single fix applied:
//
//   `Certificate.rsa.PSSSignature.concatVerify` calls the private
//   `EMSA_PSS_VERIFY` which, in zig 0.17.0-dev.1609, panics at line 1151
//     `@memmove(m_p, @as(*const [8]u8, &@splat(0)))`
//   because `m_p` has length `8 + 2*Hash.digest_length` but the source is
//   only 8 bytes ("source and destination arguments have non-equal
//   lengths").  zig 0.17.0-dev.1622 fixes this to `m_p[0..8]`, but we
//   cannot depend on the build host shipping that exact zig, and we must
//   not edit the system stdlib.  Instead we vendor `tls/Client.zig` (the
//   only call site of `PSSSignature.concatVerify`) and replace the PSS
//   branch with a corrected `pssConcatVerify` that reimplements the two
//   private helpers (`encrypt` + `EMSA_PSS_VERIFY`) via public std crypto
//   APIs.  Everything else is byte-identical to the upstream file.
...
// ── RSA-PSS verification (vendored fix) ──────────────────────────────
//
// Reimplementation of `Certificate.rsa.PSSSignature.concatVerify` + the
// private `encrypt` and `EMSA_PSS_VERIFY` helpers, using only public
// `std.crypto` APIs.  The only behavioural change vs. the upstream
// `0.17.0-dev.1609` code is the corrected slice on line 1151:
//     `@memmove(m_p[0..8], ...)`   instead of   `@memmove(m_p, ...)`
// which is exactly what zig `0.17.0-dev.1622` ships.  Keeping this here
// (rather than in the system stdlib) makes the fix self-contained and
// independent of the build-host zig patch level.

/// RSA primitive: `m = sig^e mod n` (RFC 3447 RSAVP1, used by
/// `concatVerify` to recover the encoded signature).
/// `Fe` is private to `Certificate.rsa`, so we recover it via `@TypeOf`
/// on the public key's exponent field.
fn rsaEncrypt(
    comptime modulus_len: usize,
    msg: *const [modulus_len]u8,
    public_key: Certificate.rsa.PublicKey,
) error{MessageTooLong}![modulus_len]u8 {
    const Fe = @TypeOf(public_key.e);
    const n_mod = public_key.n;
    const m_fe = Fe.fromBytes(n_mod, msg, .big) catch return error.MessageTooLong;
    const c_fe = n_mod.powPublic(m_fe, public_key.e) catch unreachable;
    var res: [modulus_len]u8 = undefined;
    c_fe.toBytes(&res, .big) catch unreachable;
    return res;
}

/// MGF1 mask generation (RFC 3447 B.2.1).
fn mgf1(
    comptime Hash: type,
    out: []u8,
    seed: *const [Hash.digest_length]u8,
    len: usize,
) error{InvalidSignature}![]u8 {
    var counter: u32 = 0;
    var idx: usize = 0;
...
/// EMSA-PSS-VERIFY (RFC 3447 9.1.6) with the `m_p[0..8]` fix applied.
fn emsaPssVerify(
    msg: []const []const u8,
    em: []const u8,
    em_bit: usize,
    ...
    if (em_bit >= 1 << 61) return error.InvalidSignature;

    const em_len = ((em_bit - 1) / 8) + 1;
    ...
    if (em_len < Hash.digest_length + s_len + 2) return error.InvalidSignature;

    if (em[em.len - 1] != 0xbc) return error.InvalidSignature;

    const masked_db = em[0..(em_len - Hash.digest_length - 1)];
    const h = em[(em_len - Hash.digest_length - 1)..(em_len - 1)][0..Hash.digest_length];

    const zero_bits = em_len * 8 - em_bit;
    ...
    if (mgf_len > mgf_out_buf.len) return error.InvalidSignature; // Modulus > 4096 bits
    const mgf_out = mgf_out_buf[0 .. ((mgf_len - 1) / Hash.digest_length + 1) * Hash.digest_length];
    var db_mask = try mgf1(Hash, mgf_out, h, mgf_len);
    ...
    if (db_mask[mgf_len - s_len - 2] != 0x00) return error.InvalidSignature;
    if (db_mask[mgf_len - s_len - 1] != 0x01) return error.InvalidSignature;

    const salt = db_mask[(mgf_len - s_len)..];

    if (s_len > Hash.digest_length) return error.InvalidSignature;
    var m_p_buf: [8 + Hash.digest_length + Hash.digest_length]u8 = undefined;
    var m_p = m_p_buf[0 .. 8 + Hash.digest_length + s_len];
    // FIX: `m_p[0..8]` (not `m_p`) so the destination length matches the
    // 8-byte source — this is the one-line bug in zig 0.17.0-dev.1609.
    @memmove(m_p[0..8], @as(*const [8]u8, &@splat(0)));
    @memmove(m_p[8..][0..Hash.digest_length], &m_hash);
    @memmove(m_p[(8 + Hash.digest_length)..][0..salt.len], salt);
...
/// Drop-in replacement for `Certificate.rsa.PSSSignature.concatVerify`.
fn pssConcatVerify(
    comptime modulus_len: usize,
    ...
    public_key: Certificate.rsa.PublicKey,
    comptime Hash: type,
) (error{MessageTooLong} || error{InvalidSignature})!void {
    const mod_bits = public_key.n.bits();
    const em_dec = try rsaEncrypt(modulus_len, sig, public_key);
    try emsaPssVerify(msg, &em_dec, mod_bits - 1, Hash.digest_length, Hash);
}

// ── tests for the vendored RSA-PSS fix ────────────────────────────────
test "emsaPssVerify does not panic on valid EM (bug 0.17.0-dev.1609)" {
    // The bug was `@memmove(m_p, ...)` where m_p.len = 8 + 2*digest_len
    // but the source was 8 bytes. Any input that reaches step 12
    // (the @memmove) would panic. We build a *valid* RSA-PSS EM for a
    // 2048-bit key (emLen=256, SHA-256, sLen=32) so all steps 1-11 pass
    // and the @memmove at step 12 is actually executed. With the fix
    // (`m_p[0..8]`) the function returns void; the old code panics.
    const Hash = std.crypto.hash.sha2.Sha256;
    const digest_len = Hash.digest_length; // 32
    const s_len = digest_len;
    const em_len: usize = 256; // RSA-2048
    const em_bit = em_len * 8; // 2048 -> zero_bits = 0 (no high-bit check)
    const mgf_len = em_len - digest_len - 1; // 223

    // Build DB = PS(0x00...) || 0x01 || salt
    var db: [mgf_len]u8 = undefined;
    @memset(db[0 .. mgf_len - s_len - 2], 0); // PS
    db[mgf_len - s_len - 2] = 0x00;
    db[mgf_len - s_len - 1] = 0x01; // separator
    @memset(db[mgf_len - s_len ..], 0); // salt = zeros

    // mHash = Hash("hello")
    var m_hash: [digest_len]u8 = undefined;
    Hash.hash("hello", &m_hash, .{});

    // H = Hash(0x00*8 || mHash || salt)
    var m_p_buf: [8 + digest_len + s_len]u8 = undefined;
    @memset(m_p_buf[0..8], 0);
    @memcpy(m_p_buf[8..][0..digest_len], &m_hash);
    @memset(m_p_buf[8 + digest_len ..], 0);
    var h: [digest_len]u8 = undefined;
    Hash.hash(&m_p_buf, &h, .{});

    // maskedDB = DB XOR MGF1(H, mgf_len)
    // MGF1 writes ceil(mgf_len/digest_len)*digest_len bytes, so allocate enough.
    const mgf_buf_len = ((mgf_len - 1) / digest_len + 1) * digest_len;
    var mgf_out_buf: [mgf_buf_len]u8 = undefined;
    _ = try mgf1(Hash, &mgf_out_buf, &h, mgf_len);
    const mgf_out = mgf_out_buf[0..mgf_len];
    var masked_db: [mgf_len]u8 = undefined;
    for (0..mgf_len) |i| masked_db[i] = db[i] ^ mgf_out[i];

    // EM = maskedDB || H || 0xbc
    var em: [em_len]u8 = undefined;
    @memcpy(em[0..mgf_len], &masked_db);
    @memcpy(em[mgf_len..][0..digest_len], &h);
    em[em.len - 1] = 0xbc;

    // With the fix this returns void (valid signature).
    // The old `@memmove(m_p, ...)` would panic here.
    try emsaPssVerify(&.{"hello"}, &em, em_bit, s_len, Hash);
}
const CertificatePublicKey = struct {
    ...
    fn verifySignature(
        ...
        switch (scheme) {
            ...
            // RSA-PSS — diverted through the vendored `pssConcatVerify` (see
            // file header) because `Certificate.rsa.PSSSignature.concatVerify`
            // calls the private `EMSA_PSS_VERIFY` which panics in zig
            // 0.17.0-dev.1609 on a slice-length mismatch.
            ...
            => |comptime_scheme| {
                ...
                switch (modulus.len) {
                    inline 128, 256, 384, 512 => |modulus_len| {
                        ...
                        try pssConcatVerify(modulus_len, &sig, msg, key, Hash);
                    },
                    else => return error.TlsBadRsaSignatureBitCount,
                }
            },
